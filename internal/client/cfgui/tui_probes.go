...
func runEditProbes(m *Model, br *bufio.Reader, out io.Writer) error {
	for {
		...
		fmt.Fprintln(out, "先填写描述并 Build，预览满意后再启用。自然语言编译需要 ai.enabled=true，并在本机设置 CURSOR_API_KEY")
...
func addProbe(m *Model, br *bufio.Reader, out io.Writer) error {
	...
	fmt.Fprintln(out, "已添加（未启用）。输入编号展开后 Build 并预览。")
	return nil
}
...
func probeDetail(m *Model, idx int, br *bufio.Reader, out io.Writer) error {
	for {
		...
		if text := m.Previews[p.Key]; text != "" {
			fmt.Fprintln(out, "最近预览:")
			fmt.Fprintln(out, text)
		}
		fmt.Fprintln(out, "b Build  + 补充  e 启停  i 改描述  n 改名称  回车返回")
		fmt.Fprintf(out, "> ")
		line, err := br.ReadString('\n')
		if err != nil && strings.TrimSpace(line) == "" {
			if err == io.EOF {
		...
		case "b", "B":
			...
			fmt.Fprintln(out, "Build 完成，预览:")
			fmt.Fprintln(out, previewText(prev))
		case "+", "s", "S":
			fmt.Fprintf(out, "补充描述: ")
			extra, _ := readTrim(br)
			if err := m.supplementAt(idx, extra); err != nil {
				fmt.Fprintln(out, err.Error())
			} else {
				fmt.Fprintln(out, "已追加。请再 Build。")
			}
		case "e", "E":
