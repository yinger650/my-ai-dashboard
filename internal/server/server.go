...
type Server struct {
	...
	hub           *workspace.Hub
	oauth         *feishu.OAuth
	defaultTenant *workspace.Tenant
}

// New constructs a personal-edition Server. webFS may be nil when the frontend
// is served externally (e.g. Vite dev server). secretKey is a 32-byte AES key used for TOTP.
func New(cfg *config.Config, st *store.Store, log *slog.Logger, webFS fs.FS, secretKey []byte) *Server {
	s := &Server{cfg: cfg, st: st, log: log, limiter: newRateLimiter(), webFS: webFS, secretKey: secretKey, edition: EditionPersonal}
	if cfg != nil && st != nil {
		s.defaultTenant = &workspace.Tenant{Store: st, ArtifactDir: cfg.ArtifactDir}
	}
	return s
}

// NewFeishu constructs the Feishu multi-user edition. There is no default board.db.
func NewFeishu(cfg *config.Config, hub *workspace.Hub, oauth *feishu.OAuth, log *slog.Logger, webFS fs.FS, secretKey []byte) *Server {
	return &Server{
		cfg: cfg, log: log, limiter: newRateLimiter(), webFS: webFS, secretKey: secretKey,
		edition: EditionFeishu, hub: hub, oauth: oauth,
	}
}
...
func (s *Server) Router() http.Handler {
	...
	r.Get("/auth/meta", s.handleAuthMeta)
...
func (s *Server) mountPersonal(r chi.Router) {
	r.Route("/ingest/v1", s.mountIngest)
...
func (s *Server) mountIngest(r chi.Router) {
	r.Use(s.mwTokenAuth)
...
func (s *Server) mountAPIv1(r chi.Router, withTOTP bool) {
...
func (s *Server) mountFeishu(r chi.Router) {
	r.Get("/auth/feishu/login", s.handleFeishuLogin)
	r.Get("/auth/feishu/callback", s.handleFeishuCallback)
	r.Method(http.MethodPost, "/auth/logout", s.mwAdminSession(s.mwCSRF(http.HandlerFunc(s.handleLogout))))
	r.Get("/auth/session", s.handleSession)

	r.Route("/{slug:[a-z0-9]{8}}", func(r chi.Router) {
		r.Get("/", s.handleSlugRedirect)
		r.Group(func(r chi.Router) {
			r.Use(s.mwWorkspaceSlug)
			r.Route("/ingest/v1", s.mountIngest)
			s.mountAPIv1(r, false)
		})
	})
